...
def Attention3dBlockMerge(inputs,single_attention_vector = False):
...
def Attention3dBlock(inputs, single_attention_vector=False):
    # inputs.shape = (batch_size, time_steps, input_dim)
...
def AttentionModel(INPUT_DIMS=13, TIME_STEPS=20, lstm_units=64):
    ...
    attention_mul = Attention3dBlock(lstm_out)
...
def PredictWithData(data, data_yuan, name, modelname, INPUT_DIMS=13, TIME_STEPS=20):
    ...
    testX, _ = CreateDataset(data)
    _, testY = CreateDataset(data_y)
    print("testX Y shape is:", testX.shape, testY.shape)
    if len(testY.shape) == 1:
        testY = testY.reshape(-1, 1)

    model = AttentionModel(INPUT_DIMS)
    model.load_weights(modelname)
    model.summary()
    y_hat = model.predict(testX)
    testY, y_hat = XgbScheduler(data_yuan, y_hat)
    return y_hat, testY
...
def WalkForwardValidation(train, test):
    predictions = list()
    train = train.values
...
def XgbScheduler(data, y_hat):
    close = data.pop('close')
    data.insert(5, 'close', close)
    train, test = PrepareData(data, n_test=len(y_hat), n_in=6, n_out=1)
    testY, y_hat2 = WalkForwardValidation(train, test)
    return testY, y_hat2
